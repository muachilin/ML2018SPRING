...
for file in os.listdir(face_folder):
	...
	img = transform.resize(img, (200,200,3) ,mode='constant' , preserve_range =True)
	img = np.array(img)
	img = img.flatten()
...
ori_img = transform.resize(ori_img, (200,200,3) ,mode='constant' ,preserve_range =True)
ori_img = np.array(ori_img)
ori_img = np.reshape(ori_img , (1,120000))
...
recon = np.reshape(recon,(200,200,3))
recon -= np.min(recon)
recon /= np.max(recon)
