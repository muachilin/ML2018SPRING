...
def cnn_model():
    model = Sequential()
    model.add(Conv2D(64, (10, 3), input_shape = (128, 10, 1), padding = 'same') )
    model.add(PReLU())
    model.add(BatchNormalization(axis = -1))
    model.add(MaxPooling2D((2,2))) # 64 * 5
    model.add(Dropout(0.2)) 

    model.add(Conv2D(128, (10, 3), padding = 'same'))
    model.add(PReLU())
    model.add(BatchNormalization(axis = -1))
...
def train(modelname):
    ...
    model.fit(train_X,train_Y,epochs=100,batch_size=128,validation_data=(val_X,val_Y),verbose=1,callbacks=checkpoint)
